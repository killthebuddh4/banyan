...
const MUST_HAVE_ACCEPTED_INVITE_ERROR = `
  Every channel has an owner. Only the owner can invite users to the channel.
  Before a user can be added to a channel, they must have been invited by the
  owner and they must have accepted the invite.  We attempted to add a user that
  has not accepted an invite to the channel.
`;
...
export const updateChannelAddUser = ({
  ...
  if (invitation === undefined) {
    throw new DescriptiveError(
      MUST_HAVE_ACCEPTED_INVITE_ERROR,
      `Failed to add user ${userToAdd.address} to channel ${channelAddress} because the user has not accepted an invitation`,
    );
  }
